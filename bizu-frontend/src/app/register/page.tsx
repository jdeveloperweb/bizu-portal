...
export default function RegisterPage() {
    ...
    const [sendingCode, setSendingCode] = useState<{ email: boolean, whatsapp: boolean }>({ email: false, whatsapp: false });
    ...
    const handleSendCodes = async (e: React.FormEvent) => {
        ...
        try {
            // Send both codes
            setSendingCode({ email: true, whatsapp: true });
            const emailResult = await sendVerificationCode(form.email, form.name, 'EMAIL');
            const whatsappResult = await sendVerificationCode(form.phone, form.name, 'WHATSAPP');
            ...
            setLoading(false);
            setSendingCode({ email: false, whatsapp: false });
        }
    };
                            ...
                            <form onSubmit={handleSendCodes} className="space-y-5">
                                <div className="space-y-2">
                                    ...
                                    <input type="text" required value={form.name}
                                        ...
                                        className="w-full h-[58px] px-5 rounded-2xl bg-slate-50 border-2 border-transparent focus:bg-white focus:border-indigo-500 focus:ring-0 outline-none transition-all" />
                                </div>
                                <div className="space-y-2">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        <input type={show ? "text" : "password"} required value={form.password}
                                            ...
                                            className="w-full h-[58px] px-5 pr-14 rounded-2xl bg-slate-50 border-2 border-transparent focus:bg-white focus:border-indigo-500 outline-none transition-all" />
                                        <button type="button" onClick={() => setShow(!show)} className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-indigo-600">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input type={show ? "text" : "password"} required value={form.confirm}
                                        ...
                                        className={`w-full h-[58px] px-5 rounded-2xl bg-slate-50 border-2 border-transparent focus:bg-white focus:border-indigo-500 outline-none transition-all ${mismatch ? "border-red-500" : ""}`} />
                                </div>

                                <button type="submit" disabled={loading || mismatch}
                                    className="w-full h-[62px] rounded-2xl font-black text-white bg-indigo-600 hover:bg-slate-900 transition-all shadow-lg active:scale-[0.98] flex items-center justify-center gap-2">
